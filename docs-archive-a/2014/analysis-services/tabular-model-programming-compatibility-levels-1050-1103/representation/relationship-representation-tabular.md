...
ms.locfileid: "87680076"
---
# <a name="relationship-representation-tabular"></a><span data-ttu-id="7387e-102">Representação de relação (de tabela)</span><span class="sxs-lookup"><span data-stu-id="7387e-102">Relationship Representation (Tabular)</span></span>
  <span data-ttu-id="7387e-103">Uma relação é uma conexão entre duas tabelas de dados.</span><span class="sxs-lookup"><span data-stu-id="7387e-103">A relationship is a connection between two tables of data.</span></span> <span data-ttu-id="7387e-104">A relação estabelece como os dados nas duas tabelas devem ser correlacionados.</span><span class="sxs-lookup"><span data-stu-id="7387e-104">The relationship establishes how the data in the two tables should be correlated.</span></span>  
  
 <span data-ttu-id="7387e-105">Consulte [Relationship Representation (Tabular)](relationship-representation-tabular.md) para obter uma explicação detalhada sobre como criar e manipular a representação de relações.</span><span class="sxs-lookup"><span data-stu-id="7387e-105">See [Relationship Representation (Tabular)](relationship-representation-tabular.md) for a detailed explanation on how to create and manipulate the relationship representation.</span></span>  
  
## <a name="relationship-representation"></a><span data-ttu-id="7387e-106">Representação de relação</span><span class="sxs-lookup"><span data-stu-id="7387e-106">Relationship Representation</span></span>  
 <span data-ttu-id="7387e-107">Em modelos de tabela, podem ser definidas várias relações entre duas tabelas.</span><span class="sxs-lookup"><span data-stu-id="7387e-107">In tabular models, multiple relationships can be defined between two tables.</span></span> <span data-ttu-id="7387e-108">Quando são definidas várias relações entre duas tabelas, somente uma pode ser definida como a relação padrão para o modelo e é chamada de relação Ativa; todas as outras relações são chamadas de Inativas.</span><span class="sxs-lookup"><span data-stu-id="7387e-108">When multiple relationships, between two tables, are defined only one can be defined as the default relationship for the model and it is named as the Active relationship; all other relationships are named as Inactive.</span></span>  
  
### <a name="relationship-in-amo"></a><span data-ttu-id="7387e-109">Relação no AMO</span><span class="sxs-lookup"><span data-stu-id="7387e-109">Relationship in AMO</span></span>  
 <span data-ttu-id="7387e-110">Em termos de objetos AMO, todas as relações inativas têm uma representação de uma relação de mapeamento um para um com o <xref:Microsoft.AnalysisServices.Relationship> e nenhum outro objeto AMO principal é exigido; para a relação ativa outros requisitos existem e um mapeamento para o <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension> também é exigido.</span><span class="sxs-lookup"><span data-stu-id="7387e-110">In terms of AMO objects all inactive relationships have a representation of a one to one mapping relationship with <xref:Microsoft.AnalysisServices.Relationship> and no other main AMO objects are required; for the active relationship other requirements exist and a mapping to the <xref:Microsoft.AnalysisServices.ReferenceMeasureGroupDimension> is also required.</span></span>  
  
 <span data-ttu-id="7387e-111">Os snippets de códigos a seguir mostram como criar uma relação em modelos de tabela, como ativar uma relação e como definir uma chave primária em uma tabela (além de "RowNumber").</span><span class="sxs-lookup"><span data-stu-id="7387e-111">The following code snippets show how to create a relationship in Tabular models, how to activate a relationship and how to define a primary key in a table (other than "RowNumber").</span></span> <span data-ttu-id="7387e-112">Para criar uma relação ativa, uma chave primária precisa ser definida na tabela de chave primária - PKTableName - da relação (o lado um da relação). O exemplo mostrado aqui cria a chave primária no PKColumnName se nenhuma chave primária tiver sido definida nesta coluna.</span><span class="sxs-lookup"><span data-stu-id="7387e-112">To create an active relationship a primary key need to be defined in the primary key table - PKTableName - of the relationship (the one side of the relationship), the sample shown here creates the primary key on the PKColumnName if no primary key is defined in this column.</span></span> <span data-ttu-id="7387e-113">Podem ser criadas relações inativas sem a necessidade de ter uma chave primária na coluna de chave primária.</span><span class="sxs-lookup"><span data-stu-id="7387e-113">Inactive relationships can be created without the need of having a primary key on the primary key column.</span></span>  
...
## <a name="amo2tabular-sample"></a><span data-ttu-id="7387e-114">Exemplo de AMO2Tabular</span><span class="sxs-lookup"><span data-stu-id="7387e-114">AMO2Tabular sample</span></span>  
 <span data-ttu-id="7387e-115">No entanto, para compreender como usar o AMO para criar e manipular representações de relação, consulte o código-fonte do exemplo AMO para Tabela.</span><span class="sxs-lookup"><span data-stu-id="7387e-115">However, to have an understanding on how to use AMO to create and manipulate relationship representations see the source code of the AMO to Tabular sample.</span></span> <span data-ttu-id="7387e-116">O exemplo está disponível no Codeplex.</span><span class="sxs-lookup"><span data-stu-id="7387e-116">The sample is available at Codeplex.</span></span> <span data-ttu-id="7387e-117">Uma nota importante sobre o código: o código é fornecido apenas como um suporte aos conceitos lógicos explicados aqui; ele não deve ser usado em um ambiente de produção, nem para fins que não sejam pedagógicos.</span><span class="sxs-lookup"><span data-stu-id="7387e-117">An important note about the code: the code is provided only as a support to the logical concepts explained here and should not be used in a production environment; nor should it be used for other purpose other than the pedagogical one.</span></span>  
