...
##  <a name="model-properties"></a><a name="bkmk_model_properties"></a>Propriedades do modelo  
 **Avançado**  
  
|Propriedade|Configuração padrão|Descrição|  
|--------------|---------------------|-----------------|  
|**Ação de Build**|Compilar|Esta propriedade especifica como o arquivo se relaciona ao processo de criação e implantação. Essa configuração de propriedade tem as seguintes opções:<br /><br /> **Compilar** -uma ação de compilação normal ocorre. As definições para objetos de modelo serão gravadas no arquivo .asdatabase.<br /><br /> **Nenhum** -a saída para o arquivo. asdatabase estará vazia.|  
|**Copiar para diretório de saída**|Não copiar|Esta propriedade especifica o arquivo de origem que será copiado no diretório de saída. Essa configuração de propriedade tem as seguintes opções:<br /><br /> Não **copiar** – nenhuma cópia é criada no diretório de saída.<br /><br /> **Copy Always** -uma cópia sempre é criada no diretório de saída.<br /><br /> **Copiar se mais recente** – Uma cópia será criada somente no diretório de saída se houver alterações ao arquivo model.bim.|  
...
###  <a name="to-configure-model-property-settings"></a><a name="bkmk_conf"></a>Para definir as configurações de Propriedade do modelo  
