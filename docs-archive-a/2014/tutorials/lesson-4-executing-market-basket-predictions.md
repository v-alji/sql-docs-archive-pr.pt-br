...
# <a name="lesson-4-executing-market-basket-predictions"></a>Lição 4: Executando previsões de Market Basket
 ...
 O formulário de ** \<model> junção de previsão de seleção** da `SELECT` instrução contém três partes:  
...
## <a name="singleton-prediction-join-statement"></a>Instrução singleton PREDICTION JOIN  
 A primeira etapa é criar uma consulta singleton, usando a sintaxe **SELECT FROM \<model> preditiva Join** e fornecendo um único conjunto de valores como entrada. Segue um exemplo genérico da instrução singleton:  
...
1.  No Pesquisador de **objetos**, clique com o botão direito do mouse na instância do [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] , aponte para **nova consulta**e clique em **DMX** para abrir o editor de consultas.  
