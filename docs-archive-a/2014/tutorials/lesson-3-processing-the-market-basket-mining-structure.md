...
```  
 ...
 As linhas seguintes do código especificam as colunas definidas pela estrutura de mineração. É preciso listar cada coluna na estrutura de mineração, e cada coluna deve mapear para uma coluna contida nos dados da consulta de origem. Você pode utilizar `SKIP` para ignorar colunas que existem nos dados de origem, mas não existem na estrutura de mineração. Para obter mais informações sobre como usar o `SKIP` , consulte [inserir em &#40;DMX&#41;](/sql/dmx/insert-into-dmx).  
