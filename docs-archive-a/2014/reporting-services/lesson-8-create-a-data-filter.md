...
6.  Clique no botão de expressão ao lado do campo **valor** , clique em **parâmetros** na área **categoria** e clique duas vezes `productid` na área **valores** . O campo **Definir expressão para: Valor** agora deve conter uma expressão semelhante a **=Parameters!productid.Value**.  
...
2.  Crie uma nova função que aceite um parâmetro, `productid` , do tipo inteiro, retorne um `datatable` objeto e faça o seguinte.  
