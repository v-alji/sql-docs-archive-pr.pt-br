...
4.  Adicione uma entrada para sua extensão de processamento de dados, que inclui um elemento de **extensão** com valores para os `Name` `Type` atributos, e `Visible` . A sua entrada poderia ser assim:  
...
-   Adicione a seguinte entrada ao arquivo de RSReportDesigner.config sob o elemento **Designer** , substituindo o `Name` atributo pelo nome que você forneceu nas entradas anteriores.  
