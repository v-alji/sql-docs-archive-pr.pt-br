...
## <a name="configuring-reporting-rervices-extended-protection"></a>Configurando a proteção estendida do Reporting Services
 O `rsreportserver.config` arquivo contém os valores de configuração que controlam o comportamento da [!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)] proteção estendida.

 Para obter mais informações sobre como usar e editar o `rsreportserver.config` arquivo, consulte [arquivo de configuração RSReportServer](../report-server/rsreportserver-config-configuration-file.md). As configurações da proteção estendida também podem ser alteradas e inspecionadas com APIs do WMI. Para obter mais informações, veja [Método SetExtendedProtectionSettings &#40;WMI MSReportServer_ConfigurationSetting&#41;](../wmi-provider-library-reference/configurationsetting-method-setextendedprotectionsettings.md).
