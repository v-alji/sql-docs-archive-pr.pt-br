...
2.  Arraste o campo Vendas até o painel **Valores** . Sales representa a quantidade de vendas da subcategoria. O painel **valores** é exibido `[Sum(Sales)]` porque o gráfico exibe a agregação de cada produto.  
...
5.  Adicional Para especificar quantas casas decimais o rótulo deve mostrar, digite `#PERCENT{Pn}` onde *n* é o número de casas decimais a serem exibidas. Por exemplo, para não exibir casas decimais, digite `#PERCENT{P0}` .  
