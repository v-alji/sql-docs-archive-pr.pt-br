...
> [!NOTE]  
>  A sintaxe do caminho do elemento é independente do namespace. Para usar namespaces em um caminho de elemento, use a sintaxe de consulta XML que inclui um `ElementPath` elemento XML descrito em [sintaxe de consulta XML para dados de relatório xml &#40;&#41;do SSRS ](report-data-ssrs.md).  
...
|`Attribute`|Um par de nome-valor dentro do `ElementNode`. Por exemplo, no nó do elemento \<Customer ID="1"> , `ID` é um atributo e `@ID(Integer)` retorna "1" como um tipo inteiro no campo de dados correspondente `ID` .|  
|`Value`|O valor do elemento. `Value` pode ser usado apenas no último `ElementNode` no caminho do elemento. Por exemplo, como \<Return> é um nó folha, se você o inclui no final de um caminho de elemento, o valor de `Return {@}` é `Chair` .|  
