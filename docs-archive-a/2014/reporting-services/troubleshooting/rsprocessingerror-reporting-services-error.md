...
-   O processamento de *\<report item type>* para o *\<report item name>* não pode ser executado. Não é possível comparar dados de tipos *\<type>* e *\<type>* . Verifique o tipo de dados retornado pelo *\<report item name>* .  
  
-   Falha ao avaliar o *\<property name>* .  
  
-   Falha ao avaliar o *\<property name>* . Ele faz referência a um campo de conjunto de uma que tem um erro: *\<error string>* .  
...
-   `Previous`, `RunningValue` ou as `RowNumber` funções de agregação usadas nas células de dados de *\<report item type>* ' *\<report item name>* ' referem-se a escopos de agrupamento nas colunas e nas linhas do *\<report item type>* . Os parâmetros de escopo de `Previous` All `RunningValue` e `RowNumber` funções de agregação dentro de um *\<report item type>* podem se referir a agrupamentos de linhas ou a grupos de colunas de dados, mas não a ambos.  
