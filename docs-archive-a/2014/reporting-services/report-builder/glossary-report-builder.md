...
|aggregate|Um valor resumido que compreende vários valores individuais.|  
...
|expressão complexa|Uma expressão que não pode ser representada como uma única referência de campo interna e é exibida na superfície de design como <\<Expr>>.|  
...
|object|Um dos componentes de um banco de dados, como uma tabela, um índice ou um procedimento armazenado.|  
...
|schema|No padrão do SQL-92, uma coleção de objetos de banco de dados que são se propriedade de um único usuário e formam um único namespace. Um namespace é um conjunto de objetos que não podem ter nomes duplicados.|  
