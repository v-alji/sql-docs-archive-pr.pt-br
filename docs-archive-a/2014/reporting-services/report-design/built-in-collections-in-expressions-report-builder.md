...
##  <a name="using-collection-syntax-in-an-expression"></a><a name="Syntax"></a> Usando sintaxe de coleção em uma expressão  
 Para fazer referência a uma coleção de uma expressão, use a [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] sintaxe padrão para um item em uma coleção. A tabela a seguir mostra exemplos de sintaxe de coleção.  
