...
2.  ** @StartDate e @EndDate parâmetros** o parâmetro @StartDate é tipo de dados `DateTime` . O aviso Data de Início é exibido ao lado da caixa de texto. Para modificar a data, digite uma nova data na caixa de texto ou use o controle de calendário.  
  
     O parâmetro @EndDate é exibido ao lado de @StartDate .  
  
3.  ** @Subcategory parâmetro** o parâmetro @Subcategory é tipo de dados `Text` . Como @Subcategory o tem uma lista de valores disponíveis, os valores válidos aparecem em uma lista suspensa. Você deve escolher os valores nessa lista. Como @Subcategory é de valores de multivalor, é exibida uma opção **selecionar tudo** que permite limpar todos e selecionar todos os valores na lista.  
  
4.  ** @ShowAllRows parâmetro** o parâmetro @ShowAllRows é tipo de dados `Boolean` . Use os botões de opção para especificar `True` ou `False`.  
