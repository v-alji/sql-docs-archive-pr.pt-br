...
##  <a name="including-references-to-commonly-used-functions"></a><a name="Common"></a>Incluindo referências a funções comumente usadas  
 Use a caixa de diálogo **Expressão** para exibir uma lista categorizada de funções comuns internas do [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]. Quando você expande **Funções Comuns** e clica em uma categoria, o painel **Item** exibe a lista de funções incluídas em uma expressão. As funções comuns incluem classes dos [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] <xref:System.Math> namespaces e das funções de biblioteca de <xref:System.Convert> tempo de [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] execução. Por conveniência, é possível exibir as funções usadas mais frequentemente na caixa de diálogo **Expressão** , onde elas são listadas por categoria: Texto, Data e Hora, Matemática, Inspeção, Fluxo do Programa, Agregação, Financeira, Conversão e Diversas. As funções usadas com menor frequência não são exibidas na lista, mas ainda podem ser usadas em uma expressão.  
