...
-   [URLReservations (arquivo de RSReportServer.config)](#bkmk_URLReservations)  
  
-   [Autenticação (arquivo de RSReportServer.config)](#bkmk_Authentication)  
  
-   [Serviço (arquivo de RSReportServer.config)](#bkmk_service)  
  
-   [Configuração geral de extensão (s) da interface do usuário de entrega](#bkmk_ui)  
  
-   [Modo nativo de extensões (arquivo de RSReportServer.config)](#bkmk_extensions)  
...
-   [MapTileServerConfiguration (arquivo de RSReportServer.config)](#bkmk_MapTileServer)  
...
-   [Configurações de informações de dispositivos para extensões de renderização &#40;Reporting Services&#41;](../device-information-settings-for-rendering-extensions-reporting-services.md)  
 ...
 Para obter informações sobre os atributos do **\<Extension>** elemento filho em **\<Render>** , consulte o seguinte:  
...
##  <a name="maptileserverconfiguration-rsreportserverconfig-file"></a><a name="bkmk_MapTileServer"></a> MapTileServerConfiguration (RSReportServer.config file)  
 `MapTileServerConfiguration`define as definições de configuração para os [!INCLUDE[msCoName](../../includes/msconame-md.md)] Serviços Web do Bing Maps que fornecem um plano de fundo de bloco para um item de relatório de mapa em um relatório publicado em um servidor de relatório. Todos os elementos filho são necessários.  
