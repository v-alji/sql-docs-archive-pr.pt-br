...
## <a name="about-database-versions"></a>Sobre as versões de banco de dados  
 No [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)], não há informações explícitas sobre a versão do banco de dados disponíveis. No entanto, como as versões de banco de dados sempre são sincronizados com as versões de produto, você pode usar as informações de versão do produto para saber quando a versão de banco de dados foi alterada. As informações de versão do produto para o [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] são indicadas por meio de informações de versão do arquivo que aparecem nos arquivos de log, nos cabeçalhos de todas as chamadas SOAP e quando você se conecta à URL do servidor de relatório (por exemplo, quando você abre um navegador para o http://localhost/reportserver) .  
