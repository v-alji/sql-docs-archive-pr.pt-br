...
3.  Em c:\MySample, crie o arquivo `CalendarAware.cs` e copie o código de exemplo C# (abaixo) nele.  
  
4.  Em c:\MySample, crie o arquivo `calendars.txt` e copie o código de exemplo no arquivo.  
  
5.  Em c:\MySample, crie o arquivo `calendars.ar-SA.txt` e copie o seguinte nele:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  Em c:\MySample, crie o arquivo `calendars.ja.txt` e copie o seguinte nele:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  Em c:\MySample, crie o arquivo `calendars.``zh-CN.txt` e copie o seguinte nele:  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  Em c:\MySample, crie o arquivo `build.com` e copie o seguinte nele:  
...
9. Compile os assemblies satélites executando a compilação de arquivo no prompt de comando.  
  
10. Compile o código de exemplo a partir do prompt de linha de comando executando o seguinte:  
...
11. Copie o código de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `Install.sql` no diretório de exemplo.  
  
12. Se o exemplo tiver sido instalado em um diretório diferente de `C:\MySample\`, edite o arquivo `Install.sql` conforme indicado para apontar para esse local.  
  
13. Implante o assembly e o procedimento armazenado executando o seguinte  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. Copie o script de comando de teste do [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `test.sql` no diretório de exemplo.  
  
15. Execute o script de teste com o seguinte comando  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. Copie o script de limpeza [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `cleanup.sql` no diretório de exemplo.  
  
17. Execute o script com o seguinte comando  
...
```  
 ...
 Este é o arquivo `calendars.txt` necessário para definir os calendários.  
...
```  
 ...
 Este é o script de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`), que implanta os assembly e cria as funções de conversão dentro do banco de dados; ele também cria uma tabela de dados de exemplo.  
...
```  
 ...
 Este é `test.sql`, que testa o exemplo executando as funções em vários calendários.  
...
```  
 ...
 O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove os assemblies, as funções e a tabela do banco de dados.  
