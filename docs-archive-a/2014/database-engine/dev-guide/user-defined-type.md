---
title: Tipo definido pelo usuário | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87681114"
---
# <a name="user-defined-type"></a><span data-ttu-id="8dd9f-102">Tipo definido pelo usuário</span><span class="sxs-lookup"><span data-stu-id="8dd9f-102">User Defined Type</span></span>
  <span data-ttu-id="8dd9f-103">O exemplo Tipo de dados definidos pelo usuário mostra a criação e o uso de um tipo de dados simples definido pelo usuário, tanto do Transact-SQL quanto de um aplicativo cliente usando `System.Data.SqlClient`.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-103">The User Defined Data Type sample shows the creation and use of a simple user-defined data type from both Transact-SQL and a client application using `System.Data.SqlClient`.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="8dd9f-104">Pré-requisitos</span><span class="sxs-lookup"><span data-stu-id="8dd9f-104">Prerequisites</span></span>  
 <span data-ttu-id="8dd9f-105">Para criar e executar este projeto, o software a seguir deve estar instalado:</span><span class="sxs-lookup"><span data-stu-id="8dd9f-105">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="8dd9f-106">ou [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-106">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="8dd9f-107">É possível obter o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express gratuitamente no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [de Documentação e Amostras do](https://www.microsoft.com/download/details.aspx?id=42299)Express</span><span class="sxs-lookup"><span data-stu-id="8dd9f-107">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="8dd9f-108">O banco de dados AdventureWorks que está disponível no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [do](https://archive.codeplex.com/?p=SqlServerSamples)Developer</span><span class="sxs-lookup"><span data-stu-id="8dd9f-108">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="8dd9f-109">.NET Framework SDK 2.0 ou posterior ou Microsoft Visual Studio 2005 ou posterior.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-109">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="8dd9f-110">Você pode obter o .NET Framework SDK gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-110">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="8dd9f-111">Além disso, as seguintes condições devem ser atendidas:</span><span class="sxs-lookup"><span data-stu-id="8dd9f-111">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="8dd9f-112">A instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando deve ter a integração CLR habilitada.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-112">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="8dd9f-113">Para habilitar a integração CLR, execute as etapas a seguir:</span><span class="sxs-lookup"><span data-stu-id="8dd9f-113">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="8dd9f-114">Habilitando integração CLR</span><span class="sxs-lookup"><span data-stu-id="8dd9f-114">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="8dd9f-115">Execute os seguintes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="8dd9f-115">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="8dd9f-116">Para habilitar o CLR, é necessário ter a permissão `ALTER SETTINGS` de nível de servidor, que é mantida implicitamente por membros das funções de servidor fixas `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-116">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="8dd9f-117">O banco de dados AdventureWorks deve estar instalado na instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-117">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="8dd9f-118">Se você não for um administrador da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instância que está usando, deverá ter um administrador para conceder a permissão **CreateAssembly** para concluir a instalação.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-118">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="8dd9f-119">Compilando o exemplo</span><span class="sxs-lookup"><span data-stu-id="8dd9f-119">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="8dd9f-120">Crie e execute o exemplo seguindo estas instruções:</span><span class="sxs-lookup"><span data-stu-id="8dd9f-120">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="8dd9f-121">Abra um prompt de comando do Visual Studio ou do .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-121">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="8dd9f-122">Se necessário, crie um diretório para o seu exemplo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-122">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="8dd9f-123">Para este exemplo, usaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-123">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="8dd9f-124">Em c:\MySample, crie `ComplexNumber.vb` (para o exemplo do Visual Basic) ou `ComplexNumber.cs` (para o exemplo do C#) e copie o código de exemplo adequado do Visual Basic ou do C# (a seguir) no arquivo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-124">In c:\MySample, create `ComplexNumber.vb` (for the Visual Basic sample) or `ComplexNumber.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="8dd9f-125">Compile o código de exemplo no prompt de linha de comando executando uma das seguintes ações, de acordo com sua opção de linguagem.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-125">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
6.  <span data-ttu-id="8dd9f-126">Copie o código de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `Install.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-126">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
7.  <span data-ttu-id="8dd9f-127">Se o exemplo tiver sido instalado em um diretório diferente de `C:\MySample\`, edite o arquivo `Install.sql` conforme indicado para apontar para esse local.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-127">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
8.  <span data-ttu-id="8dd9f-128">Implante o assembly e o procedimento armazenado executando o seguinte</span><span class="sxs-lookup"><span data-stu-id="8dd9f-128">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
9. <span data-ttu-id="8dd9f-129">Copie o script de comando de teste do [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `test.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-129">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="8dd9f-130">Execute o script de teste com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="8dd9f-130">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
11. <span data-ttu-id="8dd9f-131">Para executar o programa de exemplo de cliente (apenas C#):</span><span class="sxs-lookup"><span data-stu-id="8dd9f-131">To run the client example program (C# only):</span></span>  
  
    -   <span data-ttu-id="8dd9f-132">Em c:\MySample, crie `Program.cs` e copie o código-fonte no arquivo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-132">In c:\MySample, create `Program.cs` and copy the source code into the file.</span></span>  
  
    -   <span data-ttu-id="8dd9f-133">Executados`: Csc /reference:ComplexNumber.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.Data.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.XML.dll /out:Client.exe /target:exe Program.cs`</span><span class="sxs-lookup"><span data-stu-id="8dd9f-133">Execute`: Csc /reference:ComplexNumber.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.Data.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.dll /reference:C:\Windows\Microsoft.NET\Framework\v2.0.50727\System.XML.dll /out:Client.exe /target:exe Program.cs`</span></span>  
  
    -   <span data-ttu-id="8dd9f-134">Teste executando: `Client AdventureWorks`</span><span class="sxs-lookup"><span data-stu-id="8dd9f-134">Test by executing: `Client AdventureWorks`</span></span>  
  
12. <span data-ttu-id="8dd9f-135">Copie o script de limpeza [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `cleanup.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-135">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
13. <span data-ttu-id="8dd9f-136">Execute o script com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="8dd9f-136">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="8dd9f-137">Exemplo de código</span><span class="sxs-lookup"><span data-stu-id="8dd9f-137">Sample Code</span></span>  
 <span data-ttu-id="8dd9f-138">As listagens de código deste exemplo são as seguintes.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="8dd9f-139">C#</span><span class="sxs-lookup"><span data-stu-id="8dd9f-139">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="8dd9f-140">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="8dd9f-140">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="8dd9f-141">Esse é o script de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) que implanta o assembly e cria o UDT no banco de dados.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-141">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the UDT in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="8dd9f-142">Esse é o `test.sql`, que testa o exemplo por meio da execução do tipo.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-142">This is `test.sql`, which tests the sample by exercising the type.</span></span>  
...
```  
 ...
 <span data-ttu-id="8dd9f-143">O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove o assembly e o tipo do banco de dados.</span><span class="sxs-lookup"><span data-stu-id="8dd9f-143">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and type from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="8dd9f-144">Consulte Também</span><span class="sxs-lookup"><span data-stu-id="8dd9f-144">See Also</span></span>  
 [<span data-ttu-id="8dd9f-145">Cenários de uso e exemplos para a integração do CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="8dd9f-145">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
