...
ms.locfileid: "87583815"
---
# <a name="result-set-sample"></a><span data-ttu-id="9827c-102">Exemplo de conjunto de resultados</span><span class="sxs-lookup"><span data-stu-id="9827c-102">Result Set Sample</span></span>
  <span data-ttu-id="9827c-103">Às vezes, é útil poder executar comandos e, ao mesmo tempo, ler os resultados de uma consulta sem abrir uma nova conexão e sem ler todos os resultados na memória.</span><span class="sxs-lookup"><span data-stu-id="9827c-103">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="9827c-104">O recurso MARS (Multiple Active Result Set) no ADO.NET 2.0 é uma tecnologia que pode ajudar a conseguir isso.</span><span class="sxs-lookup"><span data-stu-id="9827c-104">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="9827c-105">Atualmente, MARS não é implementado para o provedor em processo usado para programação de servidor.</span><span class="sxs-lookup"><span data-stu-id="9827c-105">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="9827c-106">Para solucionar essa limitação, você pode usar cursores de servidor.</span><span class="sxs-lookup"><span data-stu-id="9827c-106">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="9827c-107">Este exemplo demonstra como usar cursores de servidor para solucionar a falta de suporte a MARS para programação de servidor.</span><span class="sxs-lookup"><span data-stu-id="9827c-107">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
> [!CAUTION]  
>  <span data-ttu-id="9827c-108">O uso de cursores de servidor é dispendioso em termos de recursos de servidor e às vezes pode impedir que o otimizador de consultas no Microsoft SQL Server aprimore o desempenho de suas consultas.</span><span class="sxs-lookup"><span data-stu-id="9827c-108">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in Microsoft SQL Server from enhancing the performance of your queries.</span></span> <span data-ttu-id="9827c-109">Portanto, considere a recriação de seu código para usar `JOINs` sempre que possível.</span><span class="sxs-lookup"><span data-stu-id="9827c-109">Therefore, you might consider rewriting your code to use `JOINs` whenever possible.</span></span>  
  
 <span data-ttu-id="9827c-110">A API dessa classe é semelhante a um leitor de dados, exceto pelo fato de que você pode avançar ou recuar no conjunto de resultados e de que outros comandos podem ser emitidos na conexão enquanto o conjunto de resultados está aberto. Esta implementação é altamente simplificada para facilitar a compreensão do exemplo.</span><span class="sxs-lookup"><span data-stu-id="9827c-110">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="9827c-111">Uma implementação mais eficiente buscaria várias linhas para evitar um retorno do banco de dados por linha buscada.</span><span class="sxs-lookup"><span data-stu-id="9827c-111">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span> <span data-ttu-id="9827c-112">O uso dessa classe pode ter um volume de memória significativamente menor do que preencher um conjunto de dados com todos os resultados de uma consulta, o que é muito importante para programação do servidor.</span><span class="sxs-lookup"><span data-stu-id="9827c-112">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span> <span data-ttu-id="9827c-113">Esse exemplo também demonstra como usar o atributo "Permitir chamadores parcialmente confiáveis" para indicar que o assembly de conjunto de resultados é uma biblioteca que pode ser chamada com segurança de outros assemblies.</span><span class="sxs-lookup"><span data-stu-id="9827c-113">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="9827c-114">Essa abordagem é um pouco mais complexa, mas muito mais segura do que registrar o assembly de chamada por meio de permissão não segura.</span><span class="sxs-lookup"><span data-stu-id="9827c-114">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="9827c-115">É mais segura porque, com o registro do assembly de chamada como seguro, o assembly de chamada limita o impacto nos recursos do servidor e evita danos à integridade do servidor.</span><span class="sxs-lookup"><span data-stu-id="9827c-115">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="9827c-116">Pré-requisitos</span><span class="sxs-lookup"><span data-stu-id="9827c-116">Prerequisites</span></span>  
 <span data-ttu-id="9827c-117">Para criar e executar este projeto, o software a seguir deve estar instalado:</span><span class="sxs-lookup"><span data-stu-id="9827c-117">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="9827c-118">ou [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="9827c-118">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="9827c-119">É possível obter o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express gratuitamente no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [de Documentação e Amostras do](https://www.microsoft.com/sql-server/sql-server-editions-express)Express</span><span class="sxs-lookup"><span data-stu-id="9827c-119">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="9827c-120">O banco de dados AdventureWorks que está disponível no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [do](https://go.microsoft.com/fwlink/?linkid=62796)Developer</span><span class="sxs-lookup"><span data-stu-id="9827c-120">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="9827c-121">.NET Framework SDK 2.0 ou posterior ou Microsoft Visual Studio 2005 ou posterior.</span><span class="sxs-lookup"><span data-stu-id="9827c-121">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="9827c-122">Você pode obter o .NET Framework SDK gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="9827c-122">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="9827c-123">Além disso, as seguintes condições devem ser atendidas:</span><span class="sxs-lookup"><span data-stu-id="9827c-123">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="9827c-124">A instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando deve ter a integração CLR habilitada.</span><span class="sxs-lookup"><span data-stu-id="9827c-124">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="9827c-125">Para habilitar a integração CLR, execute as etapas a seguir:</span><span class="sxs-lookup"><span data-stu-id="9827c-125">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="9827c-126">Habilitando integração CLR</span><span class="sxs-lookup"><span data-stu-id="9827c-126">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="9827c-127">Execute os seguintes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="9827c-127">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="9827c-128">Para habilitar o CLR, é necessário ter a permissão `ALTER SETTINGS` de nível de servidor, que é mantida implicitamente por membros das funções de servidor fixas `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="9827c-128">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="9827c-129">O banco de dados AdventureWorks deve estar instalado na instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando.</span><span class="sxs-lookup"><span data-stu-id="9827c-129">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="9827c-130">Se você não for um administrador da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instância que está usando, deverá ter um administrador para conceder a permissão **CreateAssembly** para concluir a instalação.</span><span class="sxs-lookup"><span data-stu-id="9827c-130">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="9827c-131">Compilando o exemplo</span><span class="sxs-lookup"><span data-stu-id="9827c-131">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="9827c-132">Crie e execute o exemplo seguindo estas instruções:</span><span class="sxs-lookup"><span data-stu-id="9827c-132">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="9827c-133">Abra um prompt de comando do Visual Studio ou do .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="9827c-133">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="9827c-134">Se necessário, crie um diretório para o seu exemplo.</span><span class="sxs-lookup"><span data-stu-id="9827c-134">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="9827c-135">Para este exemplo, usaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="9827c-135">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="9827c-136">Crie um arquivo de chaves de nome forte executando o seguinte no prompt de comando.</span><span class="sxs-lookup"><span data-stu-id="9827c-136">Create a strong  name key file by executing the following at the command prompt.</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="9827c-137">Exemplo de código</span><span class="sxs-lookup"><span data-stu-id="9827c-137">Sample Code</span></span>  
 <span data-ttu-id="9827c-138">As listagens de código deste exemplo são as seguintes.</span><span class="sxs-lookup"><span data-stu-id="9827c-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="9827c-139">Este é o código para a Biblioteca `ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="9827c-139">This is the code for the Library `ResultSet.`</span></span>  
  
 <span data-ttu-id="9827c-140">C#</span><span class="sxs-lookup"><span data-stu-id="9827c-140">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="9827c-141">Este é o código para testar a biblioteca `ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="9827c-141">This is the code to test the library `ResultSet.`</span></span>  
  
 <span data-ttu-id="9827c-142">C#</span><span class="sxs-lookup"><span data-stu-id="9827c-142">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="9827c-143">Este é o script de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) que implanta os assemblies e cria os procedimentos armazenados.</span><span class="sxs-lookup"><span data-stu-id="9827c-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="9827c-144">Este é o [!INCLUDE[tsql](../../includes/tsql-md.md)] script de teste (`test.sql`) que testa o exemplo.</span><span class="sxs-lookup"><span data-stu-id="9827c-144">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] test script (`test.sql`), which tests the sample.</span></span>  
...
```  
 ...
 <span data-ttu-id="9827c-145">O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove os assemblies e os procedimentos armazenados do banco de dados.</span><span class="sxs-lookup"><span data-stu-id="9827c-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="9827c-146">Consulte Também</span><span class="sxs-lookup"><span data-stu-id="9827c-146">See Also</span></span>  
 [<span data-ttu-id="9827c-147">Cenários de uso e exemplos para a integração do CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="9827c-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
