...
> [!CAUTION]  
>  O uso de cursores de servidor é dispendioso em termos de recursos de servidor e às vezes pode impedir que o otimizador de consultas no Microsoft SQL Server aprimore o desempenho de suas consultas. Portanto, considere a recriação de seu código para usar `JOINs` sempre que possível.  
...
```  
 ...
 Este é o código para testar a biblioteca `ResultSet.`  
...
```  
 ...
 Este é o script de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) que implanta os assemblies e cria os procedimentos armazenados.  
...
```  
 ...
 O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove os assemblies e os procedimentos armazenados do banco de dados.  
