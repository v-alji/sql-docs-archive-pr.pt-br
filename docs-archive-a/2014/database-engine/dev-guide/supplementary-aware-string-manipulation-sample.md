...
ms.locfileid: "87681127"
---
# <a name="supplementary-aware-string-manipulation-sample"></a><span data-ttu-id="50edf-102">Exemplo de manipulação de cadeias de caracteres com reconhecimento de suplementares</span><span class="sxs-lookup"><span data-stu-id="50edf-102">Supplementary-Aware String Manipulation Sample</span></span>
  <span data-ttu-id="50edf-103">O exemplo do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] demonstra o processamento de cadeia de caracteres com reconhecimento de caracteres suplementares.</span><span class="sxs-lookup"><span data-stu-id="50edf-103">This sample for [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] demonstrates supplementary character aware string processing.</span></span> <span data-ttu-id="50edf-104">Este exemplo mostra a implementação de cinco funções da cadeia de caracteres do Transact-SQL que fornecem as mesmas funções de manipulação de cadeia de caracteres, como as funções internas, porém com capacidade de reconhecimento de caracteres suplementares adicionais para tratar cadeias de caracteres suplementares e Unicode.</span><span class="sxs-lookup"><span data-stu-id="50edf-104">This sample shows the implementation of five Transact-SQL string functions that provide the same string manipulation functions as the built-in functions, but with additional supplementary character-aware capability to handle both Unicode and supplementary character strings.</span></span> <span data-ttu-id="50edf-105">As cinco funções são lentes () `lefts(), rights(), subs()` e `replace_s()` que são equivalentes às funções internas `LEN(), LEFT(), RIGHT(), SUBSTRING()` e às funções de `REPLACE()` cadeia de caracteres.</span><span class="sxs-lookup"><span data-stu-id="50edf-105">The five functions are lens(), `lefts(), rights(), subs()` and `replace_s()` which are equivalent to the built-in functions `LEN(), LEFT(), RIGHT(), SUBSTRING()` and `REPLACE()` string functions.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="50edf-106">Pré-requisitos</span><span class="sxs-lookup"><span data-stu-id="50edf-106">Prerequisites</span></span>  
 <span data-ttu-id="50edf-107">Para criar e executar este projeto, o software a seguir deve estar instalado:</span><span class="sxs-lookup"><span data-stu-id="50edf-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="50edf-108">ou [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="50edf-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="50edf-109">É possível obter o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express gratuitamente no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [de Documentação e Amostras do](https://www.microsoft.com/sql-server/sql-server-editions-express)Express</span><span class="sxs-lookup"><span data-stu-id="50edf-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="50edf-110">O banco de dados AdventureWorks que está disponível no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [do](https://go.microsoft.com/fwlink/?linkid=62796)Developer</span><span class="sxs-lookup"><span data-stu-id="50edf-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="50edf-111">.NET Framework SDK 2.0 ou posterior ou Microsoft Visual Studio 2005 ou posterior.</span><span class="sxs-lookup"><span data-stu-id="50edf-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="50edf-112">Você pode obter o .NET Framework SDK gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="50edf-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="50edf-113">Além disso, as seguintes condições devem ser atendidas:</span><span class="sxs-lookup"><span data-stu-id="50edf-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="50edf-114">A instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando deve ter a integração CLR habilitada.</span><span class="sxs-lookup"><span data-stu-id="50edf-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="50edf-115">Para habilitar a integração CLR, execute as etapas a seguir:</span><span class="sxs-lookup"><span data-stu-id="50edf-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="50edf-116">Habilitando integração CLR</span><span class="sxs-lookup"><span data-stu-id="50edf-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="50edf-117">Execute os seguintes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="50edf-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="50edf-118">Para habilitar o CLR, é necessário ter a permissão `ALTER SETTINGS` de nível de servidor, que é mantida implicitamente por membros das funções de servidor fixas `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="50edf-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="50edf-119">O banco de dados AdventureWorks deve estar instalado na instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando.</span><span class="sxs-lookup"><span data-stu-id="50edf-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="50edf-120">Se você não for um administrador da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instância que está usando, deverá ter um administrador para conceder a permissão **CreateAssembly** para concluir a instalação.</span><span class="sxs-lookup"><span data-stu-id="50edf-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="50edf-121">Compilando o exemplo</span><span class="sxs-lookup"><span data-stu-id="50edf-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="50edf-122">Crie e execute o exemplo seguindo estas instruções:</span><span class="sxs-lookup"><span data-stu-id="50edf-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="50edf-123">Abra um prompt de comando do Visual Studio ou do .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="50edf-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="50edf-124">Se necessário, crie um diretório para o seu exemplo.</span><span class="sxs-lookup"><span data-stu-id="50edf-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="50edf-125">Para este exemplo, usaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="50edf-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="50edf-126">Como este exemplo exige um assembly assinado, crie uma chave assimétrica digitando o comando:</span><span class="sxs-lookup"><span data-stu-id="50edf-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
     `sn -k SampleKey.snk`  
  
4.  <span data-ttu-id="50edf-127">Compile o código de exemplo no prompt de linha de comando executando uma das seguintes ações, de acordo com sua opção de linguagem.</span><span class="sxs-lookup"><span data-stu-id="50edf-127">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
5.  <span data-ttu-id="50edf-128">Copie o código de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `Install.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="50edf-128">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
6.  <span data-ttu-id="50edf-129">Implante o assembly e o procedimento armazenado executando o seguinte</span><span class="sxs-lookup"><span data-stu-id="50edf-129">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql -v root = "C:\MySample\"`  
  
7.  <span data-ttu-id="50edf-130">Copie o script de comando de teste do [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `test.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="50edf-130">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="50edf-131">Execute o script de teste com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="50edf-131">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
9. <span data-ttu-id="50edf-132">Copie o script de limpeza [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `cleanup.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="50edf-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="50edf-133">Execute o script com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="50edf-133">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="50edf-134">Exemplo de código</span><span class="sxs-lookup"><span data-stu-id="50edf-134">Sample Code</span></span>  
 <span data-ttu-id="50edf-135">As listagens de código deste exemplo são as seguintes.</span><span class="sxs-lookup"><span data-stu-id="50edf-135">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="50edf-136">C#</span><span class="sxs-lookup"><span data-stu-id="50edf-136">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="50edf-137">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="50edf-137">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="50edf-138">Este é o script de instalação do [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) que implanta o assembly e cria as funções de reconhecimento de suplementar no banco de dados.</span><span class="sxs-lookup"><span data-stu-id="50edf-138">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the supplementary-aware functions in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="50edf-139">Esse é o `test.sql`, que testa o exemplo por meio da execução das funções.</span><span class="sxs-lookup"><span data-stu-id="50edf-139">This is `test.sql`, which tests the sample by executing the functions.</span></span>  
...
```  
 ...
 <span data-ttu-id="50edf-140">O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove o assembly e as funções do banco de dados.</span><span class="sxs-lookup"><span data-stu-id="50edf-140">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and functions from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="50edf-141">Consulte Também</span><span class="sxs-lookup"><span data-stu-id="50edf-141">See Also</span></span>  
 [<span data-ttu-id="50edf-142">Cenários de uso e exemplos para a integração do CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="50edf-142">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
