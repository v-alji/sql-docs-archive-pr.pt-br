...
ms.locfileid: "87681108"
---
# <a name="utf8-string-user-defined-data-type-udt"></a><span data-ttu-id="b0a08-102">Tipo de dados de cadeia de caracteres UTF8 definido pelo usuário (UDT)</span><span class="sxs-lookup"><span data-stu-id="b0a08-102">UTF8 String User-Defined Data Type (UDT)</span></span>
  <span data-ttu-id="b0a08-103">O exemplo UTF8String do SQL Server demonstra a implementação de um tipo de dados definido pelo usuário.</span><span class="sxs-lookup"><span data-stu-id="b0a08-103">The UTF8String sample for SQL Server demonstrates the implementation of a user-defined data type.</span></span> <span data-ttu-id="b0a08-104">Esse exemplo mostra a implementação de um tipo de dados UTF8 definido pelo usuário, que estende o sistema de tipos do banco de dados para fornecer armazenamento para valores UTF8 codificados.</span><span class="sxs-lookup"><span data-stu-id="b0a08-104">This sample shows the implementation of a UTF8 user-defined data type that extends the type system of the database to provide storage for UTF8-encoded values.</span></span> <span data-ttu-id="b0a08-105">Esse tipo também implementa código para converter a cadeia de caracteres Unicode de e para UTF8.</span><span class="sxs-lookup"><span data-stu-id="b0a08-105">This type also implements code to convert Unicode strings to and from UTF8.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="b0a08-106">Pré-requisitos</span><span class="sxs-lookup"><span data-stu-id="b0a08-106">Prerequisites</span></span>  
 <span data-ttu-id="b0a08-107">Para criar e executar este projeto, o software a seguir deve estar instalado:</span><span class="sxs-lookup"><span data-stu-id="b0a08-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="b0a08-108">ou [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="b0a08-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="b0a08-109">É possível obter o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express gratuitamente no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [de Documentação e Amostras do](https://www.microsoft.com/sql-server/sql-server-editions-express)Express</span><span class="sxs-lookup"><span data-stu-id="b0a08-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="b0a08-110">O banco de dados AdventureWorks que está disponível no [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] site [do](https://go.microsoft.com/fwlink/?linkid=62796)Developer</span><span class="sxs-lookup"><span data-stu-id="b0a08-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="b0a08-111">.NET Framework SDK 2.0 ou posterior ou Microsoft Visual Studio 2005 ou posterior.</span><span class="sxs-lookup"><span data-stu-id="b0a08-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="b0a08-112">Você pode obter o .NET Framework SDK gratuitamente.</span><span class="sxs-lookup"><span data-stu-id="b0a08-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="b0a08-113">Além disso, as seguintes condições devem ser atendidas:</span><span class="sxs-lookup"><span data-stu-id="b0a08-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="b0a08-114">A instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando deve ter a integração CLR habilitada.</span><span class="sxs-lookup"><span data-stu-id="b0a08-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="b0a08-115">Para habilitar a integração CLR, execute as etapas a seguir:</span><span class="sxs-lookup"><span data-stu-id="b0a08-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="b0a08-116">Habilitando integração CLR</span><span class="sxs-lookup"><span data-stu-id="b0a08-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="b0a08-117">Execute os seguintes comandos [!INCLUDE[tsql](../../includes/tsql-md.md)] :</span><span class="sxs-lookup"><span data-stu-id="b0a08-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="b0a08-118">Para habilitar o CLR, é necessário ter a permissão `ALTER SETTINGS` de nível de servidor, que é mantida implicitamente por membros das funções de servidor fixas `sysadmin` e `serveradmin`.</span><span class="sxs-lookup"><span data-stu-id="b0a08-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="b0a08-119">O banco de dados AdventureWorks deve estar instalado na instância do [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] que você está usando.</span><span class="sxs-lookup"><span data-stu-id="b0a08-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="b0a08-120">Se você não for um administrador da [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instância que está usando, deverá ter um administrador para conceder a permissão **CreateAssembly** para concluir a instalação.</span><span class="sxs-lookup"><span data-stu-id="b0a08-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="b0a08-121">Compilando o exemplo</span><span class="sxs-lookup"><span data-stu-id="b0a08-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="b0a08-122">Crie e execute o exemplo seguindo estas instruções:</span><span class="sxs-lookup"><span data-stu-id="b0a08-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="b0a08-123">Abra um prompt de comando do Visual Studio ou do .NET Framework.</span><span class="sxs-lookup"><span data-stu-id="b0a08-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="b0a08-124">Se necessário, crie um diretório para o seu exemplo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="b0a08-125">Para este exemplo, usaremos C:\MySample.</span><span class="sxs-lookup"><span data-stu-id="b0a08-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="b0a08-126">Em c:\MySample, crie `Utf8String.vb` (para o exemplo do Visual Basic) ou `Utf8String.cs` (para o exemplo do C#) e copie o código de exemplo adequado do Visual Basic ou do C# (a seguir) no arquivo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-126">In c:\MySample, create `Utf8String.vb` (for the Visual Basic sample) or `Utf8String.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="b0a08-127">Compile o código de exemplo no prompt de linha de comando executando uma das seguintes ações, de acordo com sua opção de linguagem.</span><span class="sxs-lookup"><span data-stu-id="b0a08-127">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
7.  <span data-ttu-id="b0a08-128">Copie o código de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `Install.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-128">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="b0a08-129">Se o exemplo tiver sido instalado em um diretório diferente de `C:\MySample\`, edite o arquivo `Install.sql` conforme indicado para apontar para esse local.</span><span class="sxs-lookup"><span data-stu-id="b0a08-129">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
9. <span data-ttu-id="b0a08-130">Implante o assembly e o procedimento armazenado executando o seguinte</span><span class="sxs-lookup"><span data-stu-id="b0a08-130">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
10. <span data-ttu-id="b0a08-131">Copie o script de comando de teste do [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `test.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-131">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
11. <span data-ttu-id="b0a08-132">Execute o script de teste com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="b0a08-132">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
12. <span data-ttu-id="b0a08-133">Copie o script de limpeza [!INCLUDE[tsql](../../includes/tsql-md.md)] em um arquivo e salve-o como `cleanup.sql` no diretório de exemplo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-133">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
13. <span data-ttu-id="b0a08-134">Execute o script com o seguinte comando</span><span class="sxs-lookup"><span data-stu-id="b0a08-134">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="b0a08-135">Exemplo de código</span><span class="sxs-lookup"><span data-stu-id="b0a08-135">Sample Code</span></span>  
 <span data-ttu-id="b0a08-136">As listagens de código deste exemplo são as seguintes.</span><span class="sxs-lookup"><span data-stu-id="b0a08-136">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="b0a08-137">C#</span><span class="sxs-lookup"><span data-stu-id="b0a08-137">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="b0a08-138">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="b0a08-138">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="b0a08-139">Esse é o script de instalação [!INCLUDE[tsql](../../includes/tsql-md.md)] (`Install.sql`) que implanta o assembly e cria o UDT no banco de dados.</span><span class="sxs-lookup"><span data-stu-id="b0a08-139">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the UDT in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="b0a08-140">Esse é o `test.sql`, que testa o exemplo por meio da execução do tipo.</span><span class="sxs-lookup"><span data-stu-id="b0a08-140">This is `test.sql`, which tests the sample by exercising the type.</span></span>  
...
```  
 ...
 <span data-ttu-id="b0a08-141">O [!INCLUDE[tsql](../../includes/tsql-md.md)] a seguir remove o assembly e o tipo do banco de dados.</span><span class="sxs-lookup"><span data-stu-id="b0a08-141">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and type from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="b0a08-142">Consulte Também</span><span class="sxs-lookup"><span data-stu-id="b0a08-142">See Also</span></span>  
 [<span data-ttu-id="b0a08-143">Cenários de uso e exemplos para a integração do CLR &#40;Common Language Runtime&#41;</span><span class="sxs-lookup"><span data-stu-id="b0a08-143">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
