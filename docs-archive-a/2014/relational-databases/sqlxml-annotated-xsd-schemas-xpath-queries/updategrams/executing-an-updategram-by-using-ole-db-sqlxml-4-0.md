---
title: Executando um updategram usando OLE DB (SQLXML 4,0) | Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "87680843"
---
# <a name="executing-an-updategram-by-using-ole-db-sqlxml-40"></a><span data-ttu-id="46042-102">Executando um diagrama de atualização usando OLE DB (SQLXML 4.0)</span><span class="sxs-lookup"><span data-stu-id="46042-102">Executing an Updategram by Using OLE DB (SQLXML 4.0)</span></span>
  <span data-ttu-id="46042-103">Este tópico fornece uma amostra funcional do usingOLE DB para executar um updategram.</span><span class="sxs-lookup"><span data-stu-id="46042-103">This topic provides a working sample of usingOLE DB to execute an updategram.</span></span>  
  
## <a name="using-icommandstream-to-set-an-xml-command"></a><span data-ttu-id="46042-104">Usando ICommandStream para definir um comando XML</span><span class="sxs-lookup"><span data-stu-id="46042-104">Using ICommandStream to Set an XML Command</span></span>  
 <span data-ttu-id="46042-105">A interface do OLE DB (versão 2,6 ou posterior) ICommandStream passa um comando como um objeto Stream, e não como uma cadeia de caracteres.</span><span class="sxs-lookup"><span data-stu-id="46042-105">The OLE DB (version 2.6 or later) ICommandStream interface passes a command as a stream object, rather than as a string.</span></span>  
  
 <span data-ttu-id="46042-106">Essa interface permite que um comando esteja em qualquer codificação compatível com o analisador XML.</span><span class="sxs-lookup"><span data-stu-id="46042-106">This interface allows a command to be in any encoding that the XML parser understands.</span></span> <span data-ttu-id="46042-107">Quando ICommand:: execute é chamado, o texto do comando é lido diretamente do fluxo e nenhuma conversão é necessária.</span><span class="sxs-lookup"><span data-stu-id="46042-107">When ICommand::Execute is called, the command text is read out of the stream directly and no conversion is required.</span></span> <span data-ttu-id="46042-108">Portanto, a execução de comandos XML usando a interface ICommandStream é mais eficiente.</span><span class="sxs-lookup"><span data-stu-id="46042-108">Therefore, executing XML commands using ICommandStream interface is more efficient.</span></span>  
  
### <a name="setting-xml-as-a-command-using-icommandstream-and-retrieving-the-results-as-an-xml-document"></a><span data-ttu-id="46042-109">Definindo XML como um comando usando ICommandStream e recuperando os resultados como um documento XML</span><span class="sxs-lookup"><span data-stu-id="46042-109">Setting XML as a Command Using ICommandStream and Retrieving the Results as an XML Document</span></span>  
 <span data-ttu-id="46042-110">A interface ICommandStream pode ser usada para definir documentos XML como um comando, e os resultados podem ser recuperados como um documento XML.</span><span class="sxs-lookup"><span data-stu-id="46042-110">The ICommandStream interface can be used to set XML documents as a command, and the results can be retrieved as an XML document.</span></span>  
  
#### <a name="executing-templates-with-xpath-queries"></a><span data-ttu-id="46042-111">Executando modelos com consultas XPath</span><span class="sxs-lookup"><span data-stu-id="46042-111">Executing Templates with XPath Queries</span></span>  
 <span data-ttu-id="46042-112">O modelo XML a seguir, que consiste em uma consulta XPath, é especificado como um comando usando ICommandStream:</span><span class="sxs-lookup"><span data-stu-id="46042-112">The following XML template consisting of an XPath query is specified as a command using ICommandStream:</span></span>  
...
```  
 ...
 <span data-ttu-id="46042-113">A consulta XPath no modelo é executada no seguinte esquema de mapeamento:</span><span class="sxs-lookup"><span data-stu-id="46042-113">The XPath query in the template is executed against the following mapping schema:</span></span>  
...
```  
 ...
 <span data-ttu-id="46042-114">A consulta retorna todos os elementos do funcionário.</span><span class="sxs-lookup"><span data-stu-id="46042-114">The query returns all of the employee elements.</span></span> <span data-ttu-id="46042-115">Com o mapeamento padrão, o **\<Person.Contact>** elemento é mapeado para a tabela Person. Contact no banco de dados AdventureWorks.</span><span class="sxs-lookup"><span data-stu-id="46042-115">With default mapping, the **\<Person.Contact>** element maps to the Person.Contact table in the AdventureWorks database.</span></span>  
  
###### <a name="to-set-xml-as-a-command-and-retrieving-result-as-an-xml-document"></a><span data-ttu-id="46042-116">Para definir XML como um comando e recuperar resultados como um documento XML</span><span class="sxs-lookup"><span data-stu-id="46042-116">To set XML as a command and retrieving result as an XML document</span></span>  
  
1.  <span data-ttu-id="46042-117">Inicialize e estabeleça uma conexão com o banco de dados.</span><span class="sxs-lookup"><span data-stu-id="46042-117">Initialize and establish a connection to the database.</span></span>  
  
2.  <span data-ttu-id="46042-118">Obtenha a interface ICommandStream no ICommand.</span><span class="sxs-lookup"><span data-stu-id="46042-118">Obtain the ICommandStream interface on ICommand.</span></span>  
  
3.  <span data-ttu-id="46042-119">Defina as propriedades de comando necessárias.</span><span class="sxs-lookup"><span data-stu-id="46042-119">Set the necessary command properties.</span></span> <span data-ttu-id="46042-120">Neste exemplo, a propriedade SSPROP_STREAM_BASEPATH específica de provedor é definida como o diretório onde são armazenados o esquema de mapeamento e os arquivos de modelo.</span><span class="sxs-lookup"><span data-stu-id="46042-120">In this example, the provider-specific property SSPROP_STREAM_BASEPATH is set to the directory where the mapping schema and the template files are stored.</span></span>  
  
4.  <span data-ttu-id="46042-121">Use ICommandStream:: SetCommandStream para especificar o fluxo do comando.</span><span class="sxs-lookup"><span data-stu-id="46042-121">Use ICommandStream::SetCommandStream to specify the command stream.</span></span> <span data-ttu-id="46042-122">Neste exemplo, o modelo XML que está sendo executado é lido de um arquivo.</span><span class="sxs-lookup"><span data-stu-id="46042-122">In this example, the XML template being executed is read from a file.</span></span> <span data-ttu-id="46042-123">Isso é útil quando você deseja executar modelos XML grandes.</span><span class="sxs-lookup"><span data-stu-id="46042-123">This is useful when you want to execute large XML templates.</span></span>  
  
5.  <span data-ttu-id="46042-124">Execute o comando XML usando ICommand:: Execute, solicitando a ID da interface IID_ISequentialStream.</span><span class="sxs-lookup"><span data-stu-id="46042-124">Execute the XML command using ICommand::Execute, requesting the IID_ISequentialStream interface ID.</span></span>  
  
6.  <span data-ttu-id="46042-125">Processe o resultado.</span><span class="sxs-lookup"><span data-stu-id="46042-125">Process the result.</span></span> <span data-ttu-id="46042-126">Neste exemplo, o XML lido do fluxo é exibido na tela.</span><span class="sxs-lookup"><span data-stu-id="46042-126">In this example, the XML read from the stream is displayed on the screen.</span></span>  
...
#### <a name="passing-parameters-to-templates"></a><span data-ttu-id="46042-127">Passando parâmetros para modelos</span><span class="sxs-lookup"><span data-stu-id="46042-127">Passing Parameters to Templates</span></span>  
 <span data-ttu-id="46042-128">Este exemplo mostra como valores de parâmetros podem ser passados para comandos XML.</span><span class="sxs-lookup"><span data-stu-id="46042-128">This example shows how parameter values can be passed to XML commands.</span></span> <span data-ttu-id="46042-129">Este modelo XML é especificado como um comando:</span><span class="sxs-lookup"><span data-stu-id="46042-129">This XML template is specified as a command:</span></span>  
...
```  
 ...
 <span data-ttu-id="46042-130">O modelo inclui uma consulta SQL.</span><span class="sxs-lookup"><span data-stu-id="46042-130">The template includes an SQL query.</span></span> <span data-ttu-id="46042-131">A consulta requer um valor para seu parâmetro ( @Title ).</span><span class="sxs-lookup"><span data-stu-id="46042-131">The query requires a value for its parameter (@Title).</span></span> <span data-ttu-id="46042-132">Se nenhum valor de parâmetro for passado, o valor padrão ("Mr.") será usado.</span><span class="sxs-lookup"><span data-stu-id="46042-132">If no parameter value is passed, the default value ("Mr.") is used.</span></span>  
  
 <span data-ttu-id="46042-133">Ao passar valores de parâmetros para um modelo, o nome e o valor do parâmetro devem ser especificados.</span><span class="sxs-lookup"><span data-stu-id="46042-133">In passing parameter values to a template, the parameter name and value both must be specified.</span></span>  
  
 <span data-ttu-id="46042-134">Este é o código:</span><span class="sxs-lookup"><span data-stu-id="46042-134">This is the code:</span></span>  
