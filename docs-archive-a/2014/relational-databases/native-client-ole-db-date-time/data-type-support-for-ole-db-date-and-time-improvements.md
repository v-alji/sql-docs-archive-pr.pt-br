...
-   DBTYPE_DATE (É um tipo de automação DATE. representado internamente como um `double`. A parte inteira é o número de dias desde 30 de dezembro de 1899 e a parte fracionária é a fração de um dia. Este tipo tem uma exatidão de 1 segundo, portanto tem uma escala efetiva de 0.)  
...
```  
 ...
 Os aplicativos que migrarem para o [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client que usam `sql_variant` e contam com a precisão limitada do `datetime` deverão ser atualizados se o esquema subjacente for atualizado para usar o `datetime2` em vez do `datetime`.  
